   ...
   {                                              /*то:                     */
    if                                            /* если псевдооперация DC */
     (                                            /* определяет константу   */
      TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[0]=='F' || TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[0] == 'E'
     )                                            /* следующее:             */
     {
...
int SDC()                                         /*подпр.обр.пс.опер.DC    */
 {
  ...
  else if (!memcmp(TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND, "E'", 2))
  {
    RAB = strtok(
      (char * ) TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND + 2, "'" );
    float f = atof(RAB);
    char temp;
    memcpy(RX.BUF_OP_RX, &f, sizeof(float));

    temp = RX.BUF_OP_RX[0];
    RX.BUF_OP_RX[0] = RX.BUF_OP_RX[3];
    RX.BUF_OP_RX[3] = temp;
    temp = RX.BUF_OP_RX[1];
    RX.BUF_OP_RX[1] = RX.BUF_OP_RX[2];
    RX.BUF_OP_RX[2] = temp;
    ...
    for ( J=0; J<=ITSYM; J++ )                    /* все метки исх.текста в */
     {                                            /* табл. T_SYM сравниваем */
      ...
      if( !strcmp ( METKA , METKA2 ) )            /* и при совпадении:      */
       {                                          /*  запоминаем значение   */
	 if (NBASRG <= 0x0f ) /*  метки в NBASRG и в сл.*/
